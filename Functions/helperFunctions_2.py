...
def load_preprocessed (session, load_pd=False, load_lfp=False):
    ...
    if load_lfp:
        ...
        return behaviour, ndata, n_time_index, n_cluster_index, n_region_index, n_channel_index, velocity, locations, node_names, frame_index_s, lfp, lfp_time, lfp_framerate
...
def get_paths(session=None, animal=None):
    ...
    """
    paths=pd.read_csv(r"\\gpfs.corp.brain.mpg.de\stem\data\project_hierarchy\data\paths.csv")
    if session is None:
        if animal is None:
            return paths
        else:
...
def get_shelterdist(locations, node_ind ):
    ...
    pixel2cm=88/1005
    distance2shelter=euclidean_distance(loc,shelterpoint,axis=1)
    distance2shelter*=pixel2cm
    return distance2shelter
