...
def process_frame(i, window_frame, lframe, frames):
    ...
    plt.suptitle(f"{target_behavior} {hf.convert_s(windowtime)}")
    ...
    ax1.axvline((lframe - window_frame) / vframerate, color='r', lw=4)  # loom
    ...
    for j, n in enumerate(ndata):
        ...
        ax2.scatter(spiketime, np.zeros_like(spiketime) + ycoords[j], color=cell_color, s=ndot)
    ...
    ax2.axvline((lframe - window_frame) / vframerate, color='r', lw=4)  # loom
...
def save_vid_from_images_array(images,ltime):
    ...
    start_time = time.time()
    write_video_moviepy(images, fr'{cachepath}\{animal}_{session}_{target_behavior}_at_{np.round(ltime/60,2)}_{fileend}.mp4')
    end_time = time.time()
