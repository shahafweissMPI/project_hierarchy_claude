# -*- coding: utf-8 -*-
"""
Created on Wed Sep 11 12:17:47 2024
...
def process_loom(lframe, ltime):
    print(f"Processing loom at {ltime} seconds")
    ...
    for i, window_frame in enumerate(around_lframe):
        ...
        img = plot_frame(frames[i], window_frame, window_time, lframe, ltime)
...
def plot_frame(frame, window_frame, window_time, lframe, ltime):
...
#         await asyncio.get_event_loop().run_in_executor(None, process_loom, lframe, ltime)

if __name__ == "__main__":
    # asyncio.run(main())
    for lframe, ltime in zip(frame_all_looms, time_all_looms):
        process_loom(lframe, ltime)
