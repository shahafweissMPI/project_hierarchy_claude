...
#animal='afm17365';sessions=['241206','241211','241203_01']#['241211']#['240522','240524','240525','240526']
...
savepath_0 = Path(rf"\\gpfs.corp.brain.mpg.de\stem\data\project_hierarchy\Figures\20250918_post_2sec_resort\{animal}\concat_PSTHs")
...
window = 5           #5 s; how much time before and after event start should be plotted
pre_window=1 
post_window=5
...
def plot_neuron_behavioral_tuning_subplots(n_spike_times, locations, frame_index_s, velocity, n_cluster_index, neuron_ID, savepath_0, behaviour, session):
    """
    ...
    path = ax_overall.scatter(y_coords_all, x_coords_all, c=speed_all, cmap='cool', 
    ...
    ax_overall.set_xlim(global_y_min, global_y_max) # Apply global limits
    ax_overall.set_ylim(global_x_min, global_x_max) # Apply global limits
    ...
    for idx, beh_name in enumerate(unique_behaviors):
        ...
        for start, stop in intervals:
            ...
            # Use scatter for color-coding by speed
            ax.scatter(current_y, current_x, c=current_speed, cmap='cool', 
                       s=5, vmin=global_speed_min, vmax=global_speed_max, zorder=1)
        ...
        ax.set_xlim(global_y_min, global_y_max) # Apply global limits
        ax.set_ylim(global_x_min, global_x_max) # Apply global limits
    ...
    # Save the plot
    save_path_str = savepath_0 / f"neuron_{neuron_ID}_session_{session}_behaviors_trajectory.png"
...
#    temporal_auto_correlation = cp.correlate(spike_timestamps, spike_timestamps, mode='full')[len(spike_timestamps)-1:]
    ...
    plt.close('all')     
    n_groups = len(unique_categories)+1
    ...
    speed_bins = np.arange(0, np.nanmax(group_speed) + 5, 5)  # add 5 to include max
    ...
    distance_bins = np.arange(0, np.nanmax(group_distance) + 2, 2)
    ...
    x_bins = np.arange(np.nanmin(x_coords), np.nanmax(x_coords) + 5, 5)
    y_bins = np.arange(np.nanmin(y_coords), np.nanmax(y_coords) + 5, 5)
...
#                       gridspec_kw={'height_ratios': [15] * nrows})
    ...
    factor=5
    rows, cols, gs, fig = pf.subplots(len(new_target_bs), gridspec=True, figsize=(29.7*factor, 21*factor))
    ...
    for i, b_name in enumerate(new_target_bs):
      ...
      #  for i, align_to_end in enumerate(align_to_end_flags):
       #     if align_to_end==True:
        ...
        psth_data = pf.psth_cond_refactored(neurondata, 
                            n_time_index, 
                            start_stop, 
                            velocity,
                            frame_index_s,
                            ...
                            return_data=True,
                            align_to_end=align_to_end,session=session)
        ...
        pf.remove_axes(axs[0]);pf.remove_axes(axs[1]);pf.remove_axes(axs[2])
    ...
    max_y = np.nanmax(ys) if ys else None
    ...
    if save_psth_plots==True:
        ...
        save_fname = session_savepath / f"{session}_cell{cluster}_ch{channel}_{region}_{base_firing}Hz.png"
...
#%% Loop through sessions
    ...
    #######    # Reduce selection to subsets.
        ...
        if plot_spk_viz==True:
            for idx,neuron_ID in tqdm(enumerate(n_cluster_index[target_neurons_ind]),desc="vizualiasing spikes",colour="green"):
                print(neuron_ID)
                plot_neuron_behavioral_tuning_subplots(n_spike_times, locations, frame_index_s, velocity, n_cluster_index, neuron_ID, savepath_0, behaviour, session)
        ...
        #        )
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = [
                    ...
                    for i, (reg, ch, nc, nd) in inputs            ]# Submit all tasks                        
    ...
    Neurons_pd=Neurons_pl.to_pandas(use_pyarrow_extension_array=True)
    
    Neurons_pd.to_pickle(str(Path((savepath_0).joinpath(f'concatanated_PSTHs_by_session.pkl'))))
    the_file=str(Path((savepath_0).joinpath(f'concatanated_PSTHs_by_session.pkl')).as_posix())
    p=pd.read_pickle(the_file)
    cell_behavior_rasters_pd=cell_behavior_rasters_df.to_pandas(use_pyarrow_extension_array=True)
    df=cell_behavior_rasters_pd
    
    cell_behavior_rasters_pd.to_pickle(str(Path(savepath_0).joinpath('concatanated_PSTHs_by_behavior.pkl')))
    # #plot and save raster for each cell
    # #plot raster
    #plot behavior limits
    #save
    print(f"savedc to {str(Path(savepath_0).joinpath('concatanated_PSTHs_by_behavior.pkl'))}")
    IPython.embed()
