...
savepath=r"\\gpfs.corp.brain.mpg.de\stem\data\project_hierarchy\Figures\20250408\afm16924\videos\subset\pups"#Where should the video be stored? (best an SSD)
video_cache=r"E:\videos"
if not os.path.exists(savepath):
    os.makedirs(savepath)
#savepath=r"E:\videos" #Where should the video be stored? (best an SSD)
animal = 'afm16924'
sessions = ['240526','240525','240524','240522']
...
#target_behaviors=['pup_run','pup_grab','pup_retrieve']

target_cells=[344,545,391,347]#
target_behaviors=['startle','turn','escape','pup_run','pup_grab','pup_retrieve']
...
def save_vid_from_images_array(images,ltime):       
    if len(target_cells)==1:
        ...
        str=rf"{savepath}\{animal}_{session}_{target_behavior}_{np.round(ltime/60,2)}_{FR_res}_{target_cells}.mp4"
    start_time = time.time()
...
if __name__ == '__main__':    
    ...
    for session in sessions:
     ...
     #   print(f"1 {np.shape(n_time_index)=}, {np.shape(ndata)=}")
        ...
        os.chdir(savepath)
        originalvideo_path={paths['video']}
        print(f'copying session video from:\n {originalvideo_path} \n to a local temp location in: {video_cache}')
        ...
        print(f'deleting session video from a local temp location:\n {tempfile}')  
