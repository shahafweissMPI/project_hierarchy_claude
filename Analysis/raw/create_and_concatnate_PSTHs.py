...
#from tqdm_joblib import tqdm_joblib  # make sure to install this package: pip install tqdm_joblib
...
############################ input Parameters#############################
animal = 'afm16924'
sessions = ['240522','240524','240525','240526'] ####['240522','240524','240525','240526']  # can be a list if you have multiple sessions
...
def plot_neuron_activity(spike_timestamps, binned_firing_rate, x_coords, y_coords, time_coords, speed, distance_to_shelter, head_direction=None,neuron_ID=None):
    """
    Calculate and plot various aspects of neuron activity.

    Parameters:
    - spike_timestamps (array): Timestamps of neuron spikes
    ...
    - head_direction (array): Mouse's head direction in radians
    """
...
#    temporal_auto_correlation = cp.correlate(spike_timestamps, spike_timestamps, mode='full')[len(spike_timestamps)-1:]
    ...
    for row, cat in enumerate(unique_categories):
        row+=1
        try:
            if cat=='escape':
                group=escape_group
    ...
    plt.tight_layout()
   # plt.show()
    plt.savefig(rf'D:\GitHub\NewGit\Project_hierarchy_stem\preprocessing\{n_cluster_index[i]}_{session}.png')
    plt.close('all')
