...
def plot_waveforms_from_PAG_columns(sorting_analyzer,unit_ids,PAG_column,savepath_0):         
     ...
     try:     
         ...
         sw.plot_unit_templates(sorting_analyzer,unit_ids=unit_ids,scalebar=True,max_spikes_per_unit=50,ncols=10,set_title=False,figsize=A2_fig_size,axis_equal=True,figtitle=f'{animal} {session} {region}')     
         save_str=f'{savepath_0}/{region}_{session}_templates.png';     plt.savefig(save_str,dpi=300)
         save_str=f'{savepath_0}/{region}_{session}_templates.svg';     plt.savefig(save_str)
         plt.close('all')
...
#         Colors = [region_to_color[label] for label in region_labels]
         
 #        Colors = [region_to_color.get(label, 'gray') for label in region_labels]
  #       IPython.embed()


        
        # Get the tab20 colormap
   #      num_colors = 8  # for example
    #     cmap = plt.get_cmap('tab20',num_colors)

        # Generate evenly spaced values in [0, 1] and convert each to a hex string
     #    hex_colors = [mcolors.to_hex(cmap(i)) for i in np.linspace(0, 1, num_colors)]

        
        # Create a mapping from region to color using the selected indices.
       
      #   region_colors = {}
       #  region_colors_strings={}
        # for region_i, idx in region_to_cmap_index.items():
         #   region_colors[region_i] = cmap(idx)
          #  region_colors_strings[region_i] = hex_colors[idx]
               
        # Build the dictionary of unit colors.
        # Each unit id (from sorting_analyzer.unit_ids or n_cluster_index) is assigned the color of its region (from n_region_index).
        # (Assuming that sorting_analyzer.unit_ids and n_region_index have the same length and corresponding order.)
       # unit_ids= unit_ids.transpose() in n_cluster_index
        ...
        #n_cluster_index = n_cluster_index[sorted_indices]
         ...
         #   unit_colors[(unit_id)] = region_colors_strings.get(region_i, 'black')
            
         # for unit_id, region_i in zip(sorting_analyzer.unit_ids, n_region_index):
         #    # Use the region's color if available; otherwise, assign a default color.
         #    unit_colors[(unit_id)] = region_colors.get(region_i, 'black')
        
            

           
         sw.plot_template_metrics(sorting_analyzer,unit_ids=unit_ids,unit_colors=unit_colors,figsize=A1_fig_size,figtitle=f'{animal} {session} {region}')
        
         save_str=f'{savepath_0}/{region}_{session}_templates_metrics.png';     plt.savefig(save_str,dpi=300)
         IPython.embed()
       
         save_str=f'{savepath_0}/{region}_{session}_templates_metrics.svg';     plt.savefig(save_str)
         plt.close('all')
...
def plot_neuron_behavioral_tuning_subplots_by_trial_sample(n_spike_times, locations, frame_index_s, velocity,
    ...
    ax_overall.set_xlim(gy_min, gy_max)
    ax_overall.set_ylim(gx_min, gx_max)
    ...
    for idx, beh_name in enumerate(filtered_behaviors, start=1):
        ...
        ax.set_xlim(gy_min, gy_max)
        ax.set_ylim(gx_min, gx_max)
...
def plot_neuron_behavioral_tuning_subplots_by_trial_time(
    ...
    ax0.add_patch(Circle(circle_center, circle_radius, color='black', fill=False, lw=1.5))
    ax0.set_title("Overall Session")
    ...
    for ax in axs[:total_axes]:
        ax.set_aspect('equal', adjustable='box')
        ax.set_xlim(global_y_min, global_y_max)
