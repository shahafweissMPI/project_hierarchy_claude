# -*- coding: utf-8 -*-
"""
📃 ./02_movie_beh_and_raster.py

🕰️  created on 2025-09-08
...
Reads a pickled dataframe with the top decoding neurons per behaviour, 
generated with `LogisticRegression/08_chaseRunRetrieve/05___top_subset_to_unit_selection_df.py`.
...
if not path_unit_selection.exists():
    print(f"WARNING:  unit selection file {path_unit_selection} does not exist")
    df_suggested_units = pd.DataFrame(columns=['mouse','session','selection_name','units'])
else:
...
def get_unit_selector_df(the_mouse:str,the_session:str,
        the_data_dict:Dict,*,
        spiketrains:Union[None,SpikeTrains]=None) -> pd.DataFrame:
    ...
    if spiketrains is None:
        ...
        spiketrains = SpikeTrains.from_spike_list(spike_times,
                                    ...
                                    isi_minimum=1/200.0, 
                                    t_start=t_start_spiketrain,
                                    t_stop=t_stop_spiketrain)
    ret_df_rows = []

    # First entry: all PAG units
    idx_contains_pag = [i for i, loc in enumerate(region_index) if 'PAG' in loc]
    pag_units = cluster_index[idx_contains_pag].tolist()
    ret_df_rows.append({
        "selection_name": "all_PAG",
        "units": pag_units,})

    df_suggested_units_filtered = df_suggested_units.query("mouse == @the_mouse and session == @the_session")
    for _, row in df_suggested_units_filtered.iterrows():
        ret_df_rows.append({
            "selection_name": row['selection_name'],
            "units": row['units'],})

    # and some test selections

    # all non-PAG units
    idx_not_pag = [i for i, loc in enumerate(region_index) if 'PAG' not in loc]
    not_pag_units = cluster_index[idx_not_pag].tolist()
    ret_df_rows.append({
        "selection_name": "all_not_PAG",
        "units": not_pag_units,})
    # PAG units, highest firing half
    spiketrains_pag = spiketrains.filter_by_unit_location('PAG')
    n_spikes = [len(train_) for train_ in spiketrains_pag.trains]
    # order by highest firing, take top half
    sorted_indices = np.argsort(n_spikes)[::-1]
    top_half_count = max(1, len(sorted_indices) // 2)
    top_half_indices = sorted_indices[:top_half_count]
    top_half_units = [spiketrains_pag.units[i] for i in top_half_indices]
    ret_df_rows.append({
        "selection_name": "PAG_top_half_firing",
        "units": top_half_units,})
    #now bottom half instead, also ordered by firing
    bottom_half_indices = sorted_indices[top_half_count:]
    bottom_half_units = [spiketrains_pag.units[i] for i in bottom_half_indices]
    ret_df_rows.append({
        "selection_name": "PAG_bottom_half_firing",
        "units": bottom_half_units,})
    return pd.DataFrame(ret_df_rows, columns=["selection_name", "units"])

def get_subselection_of_units(selection_name:str, spiketrains_full:SpikeTrains,
            unit_selection_df:pd.DataFrame) -> SpikeTrains:
    selected_units = unit_selection_df.query("selection_name == @selection_name")['units'].values[0]
    return spiketrains_full.filter_by_units(selected_units)
...
class VideoSelectorWidget(QWidget):
    ...
    def load_video_and_data(self, idx):
        ...
        self.unit_selector_df = get_unit_selector_df(mouse, session, self.data_dict)
        self.update_unit_selector()
    ...
    def update_unit_selector(self):
        # Populate unit selection combo from self.unit_selector_df
        names = self.unit_selector_df['selection_name'].tolist()
