...
(elasticnet) Single fit, includes a label for "no behaviour", corresponding to time intervals before
the pups are introduced, while the mouse is simply moving around (exploring?)

Focuses on session with hunting.
...
# Get the time of the last training sample
# all_train_times = t_behaviour[X_train_idx]
# # last is the maximum
# last_train_time = np.max(all_train_times)
# last_train_time_str = time.strftime("%H:%M:%S", time.gmtime(last_train_time))
# print(f"Time associated with the last element in the train set: {last_train_time_str} (HH:MM:SS)")
...
print("Last training time per label (HH:MM:SS):")
print(last_train_time_per_label_df[['behaviour_idx', 'behaviour_name', 'last_train_time_str']])
...
print("Computing full prediction. This might crash the system if there isn't enough RAM!")
...
y_predprob_full_4 = pipe.predict_proba(X_alltime_maxlagged[300000:400000,:].values)
y_predprob_full_5 = pipe.predict_proba(X_alltime_maxlagged[400000:500000,:].values)
y_predprob_full_6 = pipe.predict_proba(X_alltime_maxlagged[500000:,:].values)

y_predprob_full = np.concatenate([y_predprob_full_1, 
                                  y_predprob_full_2, 
                                  y_predprob_full_3, 
                                  y_predprob_full_4, 
                                  y_predprob_full_5,
                                  y_predprob_full_6], axis=0)

print("DONE! If you are reading this, the system didn't crash and predictions were successfully computed.")
#%%
...
#%%
dict_save = {
    "last_train_time_per_label": last_train_time_per_label_df,
...
print(f"Results saved to {saveaname}.")
