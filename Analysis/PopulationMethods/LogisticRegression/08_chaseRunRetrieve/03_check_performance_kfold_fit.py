...
After performing k-fold cross-validated fit of all sessions using `02_k_fold_fit_and_save.py`,
...
def get_avg_performance(_path:str,n_folds:int):
    ...
    for (class_idx, beh_label) in _class_idx_dict.items():
        ...
        for _report_dict in all_reports:
            ...
            support += _report_dict_beh['support']
        precision /= n_folds
        recall /= n_folds
        f1_score /= n_folds
        support /= n_folds
...
def plot_session_performance(df_all: pd.DataFrame, save: bool = True, show: bool = False):
    ...
    for (mouse, session), df_sub in df_all.groupby(["mouse", "session"]):
        ...
        fig.update_layout(
            title=f"Performance (k-fold avg) - Mouse: {mouse} | Session: {session}",
            barmode="group",
