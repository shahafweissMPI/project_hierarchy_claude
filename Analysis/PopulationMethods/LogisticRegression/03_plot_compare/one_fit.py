...
I include a label for "no behaviour", corresponding to time intervals before
the pups are introduced, while the mouse is simply moving around (exploring?)

Fit is identical to LogisticRegression/02/one_fit.py 

The goal here is to streamline the process and visualize summary statistics 
on the neurons involved.
...
def run_the_fit(animal_fit:str, session_fit:str, kbest:int = 8 ,
    ...
    units_PAG = spiketrains.units
    # if unit list is not empty, apply filter_by_unit
    if len(units_to_remove) > 0:
        units_keep = [u for u in units_PAG if u not in units_to_remove]
        spiketrains = spiketrains.filter_by_units(units_keep)
    ...
    for beh_idx,beh_name in dict_classindex_to_behaviour.items():
        ...
        summed_abs = np.abs(coefficients_beh_sum)
        # find idxs of top k elements
        topk = summed_abs.argsort()[-kbest:]
    ...
    all_scores_df = pd.DataFrame(all_scores_df_row, 
                columns=['class', 'class_idx', 'unit', 'coefficients_lag',\
                    'summed_coefficients', 'is_top_k'])
    # count how many units are repeated more than once, how many are unique
    scores_df_good = all_scores_df[all_scores_df['is_top_k']]
    unit_counts = scores_df_good['unit'].value_counts()
...
pipe, all_scores_df,\
    ...
    behaviour_representation_df,\
    dict_classindex_to_behaviour,\
    classification_report_df = run_the_fit(animal, session)
...
best_scores_df = all_scores_df[all_scores_df['is_top_k']]
...
#%%
# let's print some infos, for each class, print neuron and score
for class_name in best_scores_df['class'].unique():
    class_df = best_scores_df[best_scores_df['class'] == class_name]
    print(f"Class: {class_name}")
    for _, row in class_df.iterrows():
        print(f"  unit: {row['unit']}, score: {row['summed_coefficients']}")
    print("\n")

#%% Now, let's print information of units repeated in multiple classes

unit_counts = best_scores_df['unit'].value_counts()
repeated_units = unit_counts[unit_counts > 1]
print("Units repeated in multiple classes:")
for unit in repeated_units.index:
    # for each unit, print class and score
    print(f"\nunit: {unit}")
    df_unit = best_scores_df[best_scores_df['unit'] == unit]
    for _, row in df_unit.iterrows():
        print(f"    class: {row['class']}, score: {row['summed_coefficients']}")

#%%
best_scores_df['class'].unique()
...
# x values as strings "unit : 123"
heatmap_data.index = [f"unit : {i}" for i in heatmap_data.index]
#%%

# Plot with plotly

fig = px.imshow(heatmap_data, aspect="auto", color_continuous_scale="Picnic_r")
fig.update_layout(
...
#%% Now, run the fit again in two cases: removing best units
# and removing same number of non-best units
...
n_best_units = len(best_units)

all_minus_best_units = [u for u in all_units if u not in best_units]
# pick them randomly
non_best_to_remove = np.random.choice(non_best_units, size=n_best_units, replace=False)
...
pipe_nobest, all_scores_df_nobest,\
    ...
    classification_report_df_nobest = run_the_fit(animal, session, units_to_remove=best_units)

print("\nREMOVING NON-BEST UNITS\n")
...
pipe_noothers, all_scores_df_noothers,\
    ...
    classification_report_df_noothers = run_the_fit(animal, session, units_to_remove=non_best_to_remove)
...
merged = base_behaviors_df.merge(f1_nobest, on='behavior', how='left').merge(f1_noothers, on='behavior', how='left')

tidy = merged.melt(
    id_vars='behavior',
    value_vars=['f1_nobest', 'f1_noothers'],
    var_name='condition',
    value_name='f1-score'
)
...
fig = px.bar(
    ...
    color='condition',
    barmode='group',
    title='F1-score per behavior: removing best units vs removing non-best units',
    category_orders={'condition': ['high-scoring', 'random non high-scoring']},
    color_discrete_map={
