...
import warnings
 ...
 #%% user parameters
animal = 'afm16924';sessions =  ['240525','240526','240527','240529','240522', '240523_0']
#240523_hard_coded
#240523_2sec_1overlap
#animal='afm17365'; sessions=['241203_01','241206','241209','241210_01','241213_01','241211','241212']
...
#animal='afm17365';sessions=['241211','241203_01','241210_02']

 #%% default settings don't touch if you don't know what these do
...
for session in sessions:
        ...
        if paths['video'] is  not None  and  (not (paths['video']))==False: 
            ...
            (frame_index_s, 
             ...
             cut_rawData, 
             nidq_frame_index, 
             nidq_meta)= pp.cut_rawData2vframes(paths,int(paths['framechannel']),firstS=firstS,lastS=lastS,digital=True)    
        ...
        #%% Get diode signal
        csv_loom_s=[]
        #if np.isnan(paths['csv_loom'])==True:# no loom file - assuming no looms           
        ...
        if len(csv_loom_s) > 0:   
            ...
            if  diode_s_index is None:
                 print("diode_s_index is empty")
            else:
                diode_s_index=diode_s_index[np.unique(np.hstack(nidq_indices_to_remove))]    
       ...
       #     raise ValueError('spike times misaligned to frames')
        ...
        if spike_source=='ironclust':
            ...
            units_id_to_remove,analyzer,sorting,recording,df_irc=pp.load_IRC_in_SPI(paths['preprocessed'],paths['sorting_quality'], paths['sorting_spikes'], t1_s, tend_s,spikeglx_folder=Path(paths['ap']).parent,analyzer_mode='binary',recompute_anlayzers=recompute_anlayzers)
        ...
        if spike_source=='ironclust':
            ...
            cluster_index = [int(x) for x in cluster_index]
            #channels_for_clusters = [channel_map[unit] for unit in np.array(cluster_index)]
            channels_for_clusters = [
                                        channel_map[int(unit)]
                                        for unit in np.array(cluster_index)
                                        if int(unit) in channel_map
                                    ]
    ...
    #        raise ValueError('channel site/num for non IRONCLUST not implemented yet')
        ...
        if check_LFP_for_zeros==True:
          try:
             bad_times = pp.check_for_bad_times(animal,session, t1_s, tend_s)
             print (f'\ncheck_for_bad_times finished')
          except:
             print (f'\ncheck_for_bad_times failed...skiping\n')
             IPython.embed()
        ...
        #label behaviors during noisy times as a different category "ignore"
        if bad_times is not None:
            behaviours_f, start_stop_f, boris_frames_s_f, boris_frames_f, behavioural_category_f, problems = pp.label_bad_behaviours(behaviours, start_stop, boris_frames_s, boris_frames, behavioural_category, bad_times)
        else:
            print('no artifacts found in LFP')
        print(f"stoppped at label behaviors during noisy times as a different category ignore ")            
        IPython.embed()
        #%%SAVE
        print('saving...')
        ...
        # if not diode_s_index:
        #     diode_s_index=Non
        if 'diode_s_index' not in globals():
            diode_s_index = None
      ...
      #  df_movement=ds_movement.to_dataframe()
        ...
        plt.close('all')
print(f"completed all sessions N={len(session)}")