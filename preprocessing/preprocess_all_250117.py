...
csv_indices_to_remove = np.where((pd.read_csv(paths['csv_loom']) == 'Keypad7') | (pd.read_csv(paths['csv_loom']) == 'Keypad8'))[0]

#remove csv indices either through code check or manual input in the paths excel file
csv_indices_to_remove_paths=check_and_convert_variable(paths.CSV_indeces_to_remove)
if csv_indices_to_remove.size>0 or csv_indices_to_remove_paths.size>0:
    csv = np.hstack((csv_indices_to_remove_paths, csv_indices_to_remove))
    csv_loom_s = np.delete(csv_loom_s, csv_indices_to_remove_paths or csv_indices_to_remove)
...
for i in range(len(diode_s_index)):
    if (abs(diode_s_index[i] - csv_loom_s) < 0.4).any() == False:
        nidq_indices_to_remove.append(i)

#remove nidq indices either through code check or manual input in the paths excel file
nidq_indices_to_remove_paths=check_and_convert_variable(paths.diode_indeces_to_remove)
if nidq_indices_to_remove or nidq_indices_to_remove_paths.size>0:
    nidq = np.hstack((nidq_indices_to_remove, nidq_indices_to_remove_paths))
    diode_s_index = np.delete(diode_s_index, nidq_indices_to_remove or nidq_indices_to_remove_paths)
    print(f"removed diode indeces {nidq}")
...
if split_sessions:
    ...
    if paths['time_start_stop'] is None or np.isnan(paths['time_start_stop']):    
        ...
        split_t1_s=paths['time_start_stop'][0]
        split_tend_s=paths['time_start_stop'][1]
...
#  node_names, 
#  frame_index_s,
# ] = hf.load_preprocessed(session=session, load_lfp=False) #this works on tracking.npy which seems to need extract_sleap to be created 


vframerate=len(frame_index_s)/max(frame_index_s)
 
velocity, locations, node_names,Cm2Pixel,distance2shelter=pp.extract_sleap(paths['Cm2Pixel'],
                                                 paths['Shelter_xy'],
                                                 'f_back',
                                                 paths['mouse_tracking'],
                                                 paths['video'],
                                                 vframerate)
 ## PETROS   !!!!!!!!!!!!! write cm to pixel into csv per camera 
